// Test script for email parsing - tests the fixed parseEmailBody function
...
interface EmailData {
  ...
  estimatedUsage: { requestsPerMonth: number; duration: string };
  technicalDetails: { apiIntegrationMethod: string; dataProcessingLocation: string; securityMeasures: string };
...
function parseEmailBody(body: string): EmailData | null {
  ...
  if (data.authorizedOfficials) {
    data.authorizedOfficials = data.authorizedOfficials
      .filter((official: any) => official.name && official.email)
      .map((official: any) => ({
        name: official.name,
