...
import { Menu, X, Building, LogOut, Plus, Calendar, Target, Mail, MessageSquare } from 'lucide-react';
...
export const NGOHeader: React.FC = () => {
  ...
  const navigation = [
    ...
    { name: 'Broadcasts', href: '/ngo/broadcasts' },
          ...
          <nav className="hidden md:flex items-center space-x-4">
            {navigation.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className={`text-sm font-medium transition-colors duration-200 px-3 py-2 rounded-md whitespace-nowrap ${
